...
import no.nav.dokdistdittnav.consumer.rdist001.kodeverk.VarselStatus;
...
import static no.nav.dokdistdittnav.consumer.rdist001.to.VarselStatusCode.FERDIGSTILT;
		...
		if (nonNull(finnForsendelseResponse) && nonNull(finnForsendelseResponse.getForsendelseId())) {
			...
			if (nonNull(hentForsendelseResponse) && isValidForsendelse(hentForsendelseResponse, hoveddokumentLest)) {
				...
				administrerForsendelse.oppdaterForsendelse(OppdaterForsendelseRequest.builder()
						.forsendelseId(Long.valueOf(finnForsendelseResponse.getForsendelseId()))
						.varselStatus(FERDIGSTILT)
						.build());
				log.info("Oppdatert forsendelse med forsendelseId={} til varselStatus={}", finnForsendelseResponse.getForsendelseId(), VarselStatus.FERDIGSTILT);
			}
		}
